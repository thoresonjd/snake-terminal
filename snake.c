...
static const uint8_t WIDTH = 20, HEIGHT = 20;
static const uint16_t SIZE = WIDTH * HEIGHT;
static const char* SNAKE = "\x1b[0;41m ";
static const char* FOOD = "\x1b[0;46m ";
static const uint8_t SLEEP_TIME_MILLIS = 50;
...
typedef struct {
	coordinate_t body[400]; // SIZE 20 * 20
...
static coordinate_t index_to_coord(uint16_t idx) {
	coordinate_t coord;
	coord.x = idx % WIDTH;
	coord.y = idx / WIDTH;
	return coord;
...
static coordinate_t compute_food(const snake_t* const snake) {
...
static snake_t init_snake() {
	snake_t snake;
	snake.length = 1;
	snake.body[snake.length - 1] = (coordinate_t){ WIDTH / 2, HEIGHT / 2 }; 
...
static void update(snake_t* const snake, coordinate_t* const food) {
	...
	if (*direction == DIRECTION_UP && head->y > 0)
		head->y--;
	else if (*direction == DIRECTION_DOWN && head->y < HEIGHT - 1)
		head->y++;
	else if (*direction == DIRECTION_RIGHT && head->x < WIDTH - 1)
		head->x++;
	else if (*direction == DIRECTION_LEFT && head->x > 0)
		head->x--;
	// grow snake and create food if snake eats existing one
	if (head->x == food->x && head->y == food->y) {
		*food = compute_food(snake);
		snake->length++;
...
// TODO: take command line arguments for grid width and height
#if 0 // currently unused
static bool parse_uint8(const char* const arg, uint8_t* const value) {
	if (!arg || !value)
		return false;
	int64_t temp;
	if (!scanf(arg, "%ld", &temp))
		return false;
...
int main(int argc, char** argv) {
	...
	coordinate_t food = compute_food(&snake);
